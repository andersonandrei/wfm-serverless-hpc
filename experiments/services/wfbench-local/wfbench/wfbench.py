...
def cpu_mem_benchmark(cpu_threads: Optional[int] = 5,
    ...
    :rtype: List
    """
    total_mem = f"{total_mem}M" if total_mem else f"{100.0 / os.cpu_count()}%" #/ 10 to use only 10% of what they propose and not run out of memory
    ...
    mem_prog = ["stress-ng", "--vm", f"{mem_threads}",
                "--vm-bytes", f"{total_mem}"] #/ 10 to use only 10% of what they propose and not run out of memory
...
def get_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--out", default=None, help="output files name.")
    parser.add_argument("--inputs", help="output files name.")
    parser.add_argument("--workdir", help="Main directory for the workflow execution.")
    return parser


def io_read_benchmark_user_input_data_size(inputs, workflow_directory, memory_limit=None):
    if memory_limit is None:
        memory_limit = -1
    memory_limit = int(memory_limit)
    print("[WfBench] Starting IO Read Benchmark...")
    for file in inputs:
        with open(pathlib.Path(workflow_directory) / file, "rb") as fp:
...
def io_write_benchmark_user_input_data_size(outputs, workflow_directory, memory_limit=None):
    if memory_limit is None:
    ...
    for file_name, file_size in outputs.items():
        ...
        while file_size_todo > 0:
            with open(pathlib.Path(workflow_directory) / file_name, "ab") as fp:
...
def main():
    ...
    if args.out and args.inputs and args.workdir:
        ...
        io_write_benchmark_user_input_data_size(args.inputs, args.workdir, memory_limit=mem_bytes)
